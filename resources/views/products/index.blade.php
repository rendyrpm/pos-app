                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        <div class="flex items-start justify-between">
                            <div class="flex-1 min-w-0">
                                <a href="{{ route('products.show', $product) }}" class="text-indigo-600 hover:text-indigo-900 font-medium text-sm">{{ $product->name }}</a>
                            ...
                            <div class="text-right ml-3 shrink-0">
                                <p class="text-sm font-semibold text-gray-900">Rp {{ number_format($product->sell_price, 0, ',', '.') }}</p>
                                <p class="text-xs {{ $product->stock <= $product->min_stock ? 'text-red-600 font-medium' : 'text-gray-500' }} mt-0.5">Stok: {{ $product->stock }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 mt-3 pt-3 border-t border-gray-100">
                            <a href="{{ route('products.edit', $product) }}" class="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-900 font-medium py-1 px-2 rounded hover:bg-indigo-50 min-h-[36px]">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                Edit
                            </a>
                            <form action="{{ route('products.destroy', $product) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus produk {{ $product->name }}?')">
                                @csrf
                                @method('DELETE')
